...
public final class CampfireRecipesLoader
{
    /**
     * Load Smoking Recipes
     */
    public static void LoadCampfireRecipes()
    {
        ConfigSettings config = Main.GetInstance().GetConfigSettings();
        for(String recipe : config.GetCampfireRecipes())
        {
            ...
            if(resultItem == null)
                continue;
            CampfireRecipe campfireRecipe = GetRecipe(config, recipe, resultItem);
            if(campfireRecipe == null)
                continue;
            int bookInventorySlot = config.GetRecipeValue(Integer.class, recipe, "campfire.result.id-book");
            RecipesManager.GetInstance().AddRecipe(recipe, new CustomRecipe(campfireRecipe, bookInventorySlot, recipe));
        }
    }
    ...
    {
        String craftMaterial = config.GetRecipeValue(String.class, recipeName, "campfire.result.id");
        if(craftMaterial == null || Material.getMaterial(craftMaterial) == null)
            return null;
        int resultItemAmount = config.GetRecipeValue(Integer.class, recipeName, "campfire.result.id-amount"); //Todo: this may throw exception
        if(resultItemAmount <= 0)
            return null;
        ItemStack resultItem = new ItemStack(Material.getMaterial(craftMaterial), resultItemAmount);
        ItemMeta resultItemMeta = resultItem.getItemMeta();

        String craftCustomName = config.GetRecipeValue(String.class, recipeName, "campfire.result.name");
        if(craftCustomName != null && !craftCustomName.isEmpty())
            resultItemMeta.setDisplayName(craftCustomName.replace("&", "§"));

        String craftCustomLore = config.GetRecipeValue(String.class, recipeName, "campfire.result.lore");
     ...
     */
    private static CampfireRecipe GetRecipe(ConfigSettings config, String recipeName, ItemStack result)
    {
        String burnMaterial = config.GetRecipeValue(String.class, recipeName, "campfire.slot.1");
        if(burnMaterial == null || burnMaterial.isEmpty())
            return null;
        Material material = Material.getMaterial(burnMaterial);
        if(material == null)
            return null;
        int cookingtime = config.GetRecipeValue(Integer.class, recipeName, "campfire.result.cooking-time");
		@SuppressWarnings("deprecation")
		CampfireRecipe recipe = new CampfireRecipe(NamespacedKey.randomKey(), result, material, 1, cookingtime);
        return recipe;
    }
    ...
    {
        Set<String> enchantSet = config.GetSection(recipeName, "campfire.result.enchantments");
        if(enchantSet == null)
            return;
        for (String enchant : enchantSet)
        {
            String enchantName = config.GetRecipeValue(String.class, recipeName, "campfire.result.enchantments." + enchant + ".enchant");
            ...
            if(enchantment == null)
                continue;
            int enchantLevel = config.GetRecipeValue(Integer.class, recipeName, "campfire.result.enchantments." + enchant + ".enchant-level");
            if(enchantLevel <= 0)
                continue;
